...
from Vjezbe08.app.binary_file import BinaryFile
...
class SequentialFile(BinaryFile):
    ...
    def get_by_id(self, id):
        block_idx, rec_idx = self.find_by_id(id)
        with open(self.filename, 'rb') as f:
            f.seek(block_idx * self.block_size)
            block = self.read_block(f)
            return block[rec_idx]
    ...
    def reorganization(self):
        with open(self.filename, 'rb+') as f:
            while True:
                if not (block := self.read_block(f)):
                    break

                for rec in block:
                    if rec.get("status") == 2:
                        self.delete_by_id(rec.get("id"))
