...
public class Example04_DDL_DML_QL {
	...
	private static void insertRow() throws SQLException {
		...
		String command = "insert into " + table + " (spr, sfp, rukfp, nafp, datp) values (?, ?, ?, ?, to_date(?, 'dd.MM.yyyy.'))";
	...
	private static void deleteRow() throws SQLException {
		...
		String command = "delete from " + table + " where spr=? and sfp=?";
	...
	private static void modifyRow() throws SQLException {
		...
		String command = "update " + table + " set rukfp=?, nafp=?, datp=to_date(?, 'dd.MM.yyyy.') where spr=? and sfp=?";
