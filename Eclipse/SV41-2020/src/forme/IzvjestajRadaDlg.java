...
public class IzvjestajRadaDlg extends JDialog {

	private static final long serialVersionUID = 3342903461967460950L;
	...
	private LocalDate odabraniDatumOd;
	private LocalDate odabraniDatumDo;
	
	public IzvjestajRadaDlg(JFrame parent, Zaposleni zaposleni) {
		...
		gbc_lblDatumOd.insets = new Insets(10, 10, 5, 5);
		...
		gbc_lblDatumDo.insets = new Insets(0, 10, 5, 5);
		...
		JButton btnPrikazi = new JButton("Prikaži izvještaj");
		btnPrikazi.addActionListener(new ActionListener() {	
			...
			public void actionPerformed(ActionEvent e) {
				if (!ispravniDatumi())
					...
					int broj = 0;
					if (zaposleni.getUloga().equals(Uloge.PREDAVAC)) {
						predavac = (Predavac) zaposleni;
						for (TerminTesta termin : predavac.getTermini()) {
							LocalDate datumOdrzavanja = termin.getDatumVrijeme().toLocalDate();
							if ( datumOdrzavanja.isAfter(odabraniDatumOd) && datumOdrzavanja.isBefore(odabraniDatumDo) )
								broj += 1;
						}
						lblIzvjestaj.setText("<html>Predavač " + predavac.getIme() + " " + predavac.getPrezime() + " je organizovao " + broj + 
								" testova <br> u periodu od " + txtDatumOd.getText() + " do " + txtDatumDo.getText() + "</html>");
					}

					else if (zaposleni.getUloga().equals(Uloge.SEKRETAR)) {
						sekretar = (Sekretar) zaposleni;
						for (Zahtjev zahtjev : sekretar.getObradjeniZahtjevi())
							if ( zahtjev.getDatumObrade().isAfter(odabraniDatumOd) && zahtjev.getDatumObrade().isBefore(odabraniDatumDo) )
								broj += 1;
						lblIzvjestaj.setText("<html>Sekretar " + sekretar.getIme() + " " + sekretar.getPrezime() + " je obradio " + broj + 
								" zahtjeva <br> u periodu od " + txtDatumOd.getText() + " do " + txtDatumDo.getText() + "</html>");	
					}
